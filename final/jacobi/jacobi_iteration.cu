...
compute_on_device(const Matrix A, Matrix gpu_solution_x, const Matrix B){
	double *ssd_device;
	double ssd_host = 0.0;
	cudaMalloc((void**)&ssd_device, sizeof(double));

	Matrix A2 = allocate_matrix_on_gpu(A);
	Matrix gpu_solution_x2 = allocate_matrix_on_gpu(gpu_solution_x);
	Matrix B2 = allocate_matrix_on_gpu(B);

	int* mutex = NULL;
	cudaMalloc((void **)&mutex, sizeof(int));

	copy_matrix_to_device(A2, A);
	copy_matrix_to_device(B2, B);
	copy_matrix_to_device(gpu_solution_x2, B);
	...
	gettimeofday(&start, NULL);
	while(!done){
		cudaMemset(ssd_device, 0.0, sizeof(double));
		jacobi_iteration_kernel<<<MATRIX_SIZE/THREAD_BLOCK_SIZE,THREAD_BLOCK_SIZE>>>(A2, B2, ssd_device, mutex, gpu_solution_x2);
		cudaThreadSynchronize();	
		cudaMemcpy(&ssd_host, ssd_device, sizeof(double), cudaMemcpyDeviceToHost);

		if (sqrt(ssd_host) < THRESHOLD)
			done = 1;
		//printf("MSE during iteration %d is %f \n", iterations, ssd_host);
		iterations++;
	}
	gettimeofday(&stop, NULL);
	checkCUDAError("KERNEL ERROR");
	printf("GPU execution time = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
	copy_matrix_from_device(gpu_solution_x, gpu_solution_x2);
}

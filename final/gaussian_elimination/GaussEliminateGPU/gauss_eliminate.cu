...
{
	int i, j, k, threadblock;
	struct timeval start, stop;
	Matrix U2 = allocate_matrix_on_gpu(U);
	...
	copy_matrix_to_device(U2, U);
	
	threadblock = ceil((float)MATRIX_SIZE/(float)THREAD_BLOCK_SIZE);
	dim3 thread_blocks (THREAD_BLOCK_SIZE, 1, 1);
	dim3 grid (threadblock, 1, 1);
	
	gettimeofday(&start, NULL);

	for (k = 0; k < MATRIX_SIZE; k++){
		checkCUDAError("");
		gauss_eliminate_kernel<<< grid,thread_blocks>>>(U2.elements, k);
		cudaThreadSynchronize();	
	}
	gettimeofday(&stop, NULL);
	
	printf("GPU run time = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
	copy_matrix_from_device(U, U2);
